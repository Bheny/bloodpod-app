...
export function FeaturedCard({ article }: { article: Article }) {
  ...
  return (
    <Link
      ...
      className="block w-[230px] shrink-0 snap-start overflow-hidden rounded-2xl border-[0.5px] border-[#E5E5EA] bg-white"
    >
      {article.coverImageUrl ? (
        <div className="relative h-28">
          <Image src={article.coverImageUrl} alt="" fill sizes="230px" className="object-cover" />
        </div>
      ) : (
      ...
      <div className="p-3">
        ...
        <p className="mt-1.5 line-clamp-2 text-[12px] font-extrabold leading-tight text-ink">
          {article.title}
        </p>
        <p className="mt-1.5 text-[9px] text-ink-faint">{formatRelativeTime(article.publishedAt)}</p>
      </div>
    </Link>
  );
}
